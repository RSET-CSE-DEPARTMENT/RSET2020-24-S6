...
# Open and read the '2017.csv' file
with open('2017.csv', 'r') as file:
    ...
    for row in reader:
        ...
        if co1 != 0:
            # If a matching row exists, update the 'y1', 'y2', 'y3', 'y4', 'y5', and 'co1' columns
            sql = "UPDATE college SET y1=?,y2=?,y3=?,y4=?,y5=?,co1=? WHERE ccode=? AND coname=?"
            ...
            co = co + 1  # Increment the 'sno' value
            insert_query = "INSERT INTO college (sno, ccode, cname, loc, coname, co1, y1, y2, y3, y4, y5) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
