...
for a in range(count1):
    ...
    # Initialize a Support Vector Regression (SVR) model
    model = SVR()
    
    # Fit the SVR model to the data
    model.fit(X, y)
    
    # Make a predicted cutoff for the target year using the SVR model
    predicted_rank = model.predict([[target_year]])
    p = predicted_rank
    q = int(p)
    
    # Update the 'svr' (Support Vector Regression) column in the 'college' table with the predicted rank
    cursor.execute("UPDATE college SET svr = " + str(q) + " WHERE sno = " + str(x1) + ";")
