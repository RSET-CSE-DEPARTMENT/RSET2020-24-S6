...
for a in range(count1):
    ...
    # Initialize an MLP Regression model with specific hyperparameters
    ...
    # Update the 'mlp' (MLP Regression) column in the 'college' table with the predicted rank
    cursor.execute("UPDATE college SET mlp = " + str(q) + " WHERE sno = " + str(x1) + ";")
...
conn.close()
print("MLP Regression Prediction Done")
